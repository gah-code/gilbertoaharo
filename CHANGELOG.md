...
- Adopted the Contentful image helper for Timeline media only, adding transformed fallback `src`, responsive `srcset`, and `sizes` while preserving lazy loading, async decoding, Timeline layout/CSS, Hero behavior, Projects, article, rich-text, CMS, and CLS behavior. (`src/components/sections/TimelineSection.tsx`, `src/components/sections/TimelineSection.test.tsx`)
- Verified the deployed Batch 4.3 Timeline image delivery impact: the live bundle contains Timeline responsive image logic, Timeline image requests are transformed Contentful WebP URLs, total byte weight dropped from `5,874 KiB` to `235 KiB`, remaining estimated image-delivery savings dropped to `18 KiB`, and later image surfaces are cleared for separate Batch 4.4 planning while CLS fixes remain blocked. (`docs/planning/design-system-roadmap-current.md`, `docs/planning/TASKS.md`, `docs/planning/phase-4-image-delivery-implementation-plan.md`, `docs/design-system/design-system-snapshot-audit-current.md`)
- Captured Batch 4.4 route-specific later image-surface evidence and approved only a narrow future ArticleCard image-delivery batch; ProjectsSection, ArticlePage, and RichTextRenderer remain deferred because current route evidence does not justify implementation. (`docs/planning/phase-4-batch-4-4-later-image-surfaces.md`, `docs/planning/design-system-roadmap-current.md`, `docs/planning/TASKS.md`, `docs/planning/phase-4-image-delivery-implementation-plan.md`, `docs/design-system/design-system-snapshot-audit-current.md`)

### Verification

- `npm run lint` (passes)
- `npm run test` (passes: Vitest 33/33 files, 119/119 tests)
...
- `npm run test -- src/components/sections/TimelineSection.test.tsx` (passes: Vitest 1/1 file, 4/4 tests)
...
- Batch 4.3 live Lighthouse verification passes for Timeline image delivery: `235 KiB` total byte weight, `72,782 B` image transfer including favicon, Timeline image requests `10,148 B`, `10,241 B`, `13,049 B`, and `14,067 B`, `18 KiB` remaining estimated image-delivery savings, LCP `1.6 s`, CLS `0.009`, TBT `210 ms`, performance score `0.84`.
- Batch 4.4 route-specific Lighthouse evidence captured under `.tmp/design-system-audit/batch-4-4/`: `/articles` still has `3,859 KiB` total byte weight, `3,794,510 B` image transfer, and `2,044 KiB` estimated image-delivery savings from ArticleCard images; homepage Projects and all sitemap article detail routes do not show meaningful image-delivery savings.
